...
export const inputStartReduser = (state = initialState, action : ChangeInputValueType | ChangeInputValueErrorType ) => {
    switch (action.type){
...
export const ChangeInputStartValueAC = (inputValue: number): ChangeInputValueType => {
    return ({
        type: ACTIONS_TYPE.CHANGE_INPUT_VALUE,
        inputValue
    })
...
export const ChangeInputStartValueErrorAC = ( inputValueError: boolean): ChangeInputValueErrorType => {
    return ({
        type: ACTIONS_TYPE.CHANGE_INPUT_VALUE_ERROR,
        inputValueError
    })
